...
export default function FrameLab() {
  ...
  const onMetadata = async () => {
    ...
    if (v.requestVideoFrameCallback) {
      const probe = new Promise((res) => {
        const to = setTimeout(() => res(null), 500);
        v.requestVideoFrameCallback((now, md) => { clearTimeout(to); res(md); });
      });
      await seekTo(Math.min(dur - 0.001, 0.001));
      const md = await probe;
      if (md && md.width && md.height) { fw = md.width; fh = md.height; }
    }
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      else if (e.key === "c" || e.key === "C") { setMarkA(null); setMarkB(null); }
      ...
      <div style={{ maxWidth: 960, margin: "0 auto" }}>
        <header style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 26 }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 12 }}>
            ...
            <span style={{ fontFamily: MONO, fontSize: 11, color: MUT }}>Phase 1 · v3</span>
          </div>
          {meta && (
          ...
          <>
            ...
            <div style={{
              ...
              <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                ...
                <span style={{ fontFamily: MONO, fontSize: 12, color: MUT, padding: "7px 4px" }}>{fps} fps</span>
              ...
              <div style={{ textAlign: "right" }}>
                {deltaMs != null ? (
                  <>
                    ...
                    <div style={{ fontFamily: MONO, fontSize: 11, color: MUT, marginTop: 4 }}>
                      {deltaFrames} frames · A {pad(a, 4)} → B {pad(b, 4)} · C clears
                    </div>
                  </>
                ) : (
            ...
            <div style={{ fontFamily: MONO, fontSize: 11, color: MUT, marginTop: 14, lineHeight: 1.8 }}>
              ← → one frame · Shift ← → ten frames · Space play / pause · I mark start (A) · O mark end (B) · C clear marks
            </div>
          </>
        )}
      </div>
